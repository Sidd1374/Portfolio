git remote add origin https://github.com/Sidd1374/Portfolio.gitimport React, { useState, useEffect, useRef } from 'react';
...
const THEME = {
  builder: {
    ...
    fromTo: 'from-cyan-400 to-cyan-500',
    from600To400: 'from-cyan-600 to-cyan-400',
    bg500_10: 'bg-cyan-500/10',
    bg500_30: 'bg-cyan-500/30',
  ...
  leader: {
    ...
    fromTo: 'from-orange-400 to-orange-500',
    from600To400: 'from-orange-600 to-orange-400',
    bg500_10: 'bg-orange-500/10',
    bg500_30: 'bg-orange-500/30',
  ...
  creator: {
    ...
    fromTo: 'from-purple-400 to-purple-500',
    from600To400: 'from-purple-600 to-purple-400',
    bg500_10: 'bg-purple-500/10',
    bg500_30: 'bg-purple-500/30',
...
const PersonaSummary = ({ themeColor, persona }) => {
  ...
  return (
    ...
    >
      <div className={`bg-${themeColor}-900/10 border border-${themeColor}-500/20 rounded-xl p-4 md:p-6 backdrop-blur-sm`}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {summaryData.map((stat, index) => (
            <div key={index} className="flex flex-col items-center text-center group">
              <div className={`mb-2 text-${themeColor}-400 bg-${themeColor}-500/10 p-2 rounded-lg group-hover:scale-110 transition-transform`}>
                {stat.icon}
              </div>
...
const SkillsModal = ({ isOpen, onClose, category, skills, themeColor }) => {
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.9, y: 20 }}
            className={`relative w-full max-w-lg bg-gray-900 border border-${themeColor}-500/30 rounded-2xl overflow-hidden shadow-2xl p-6 md:p-8`}
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
              <span className={`text-${themeColor}-400`}>//</span> {category}
            </h2>
              ...
              {skills.map((skill, idx) => (
                <div key={idx}>
                  <div className="flex justify-between mb-2 text-sm font-medium text-gray-300">
                    ...
                    <span className={`text-${themeColor}-400`}>{skill.level}%</span>
                  </div>
                  <div className="h-2.5 bg-gray-800 rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      className={`h-full bg-gradient-to-r from-${themeColor}-600 to-${themeColor}-400`}
                    />
                  </div>
                </div>
              ))}
...
const CharacterStats = ({ themeColor, persona }) => {
  ...
  return (
    <section className="py-12 px-6 max-w-4xl mx-auto">
      ...
      >
        <div className={`absolute top-0 right-0 w-32 h-32 bg-${themeColor}-500/10 blur-3xl rounded-full`} />
        
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-left">
              <h3 className="text-2xl font-bold mb-2 flex items-center justify-center md:justify-start gap-2">
              <span className={`text-${themeColor}-400`}>
              ...
              <div key={idx} className="group">
                <div className="flex justify-between mb-1 text-sm font-medium">
                  <span className="text-gray-300 flex items-center gap-2">
                    <span className={`text-${themeColor}-400`}>{stat.icon}</span>
                    {stat.label}
                  </span>
                  <span className={`text-${themeColor}-400`}>{stat.value}%</span>
                </div>
...
const SkillsSection = ({ themeColor, persona, onSkillClick }) => {
  ...
  return (
    <section className="py-20 px-6 max-w-6xl mx-auto">
      <div className="mb-12 text-center">
        ...
        <div className={`h-1 w-24 mx-auto bg-${themeColor}-500 rounded-full mt-2`} />
      </div>
        ...
        {skills.map((group, idx) => (
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className={`text-${themeColor}-400 font-bold text-lg group-hover:text-white transition-colors`}>{group.category}</h3>
              <ChevronRight className={`text-${themeColor}-500 opacity-0 group-hover:opacity-100 transition-opacity`} size={18} />
            </div>
...
const AchievementsSection = ({ themeColor }) => {
  return (
    <section className="py-20 px-6 bg-gradient-to-b from-black to-gray-900/20 border-t border-gray-800">
      <div className="max-w-6xl mx-auto">
        <div className="mb-16 text-center">
          <span className={`text-${themeColor}-400 text-sm font-bold uppercase tracking-wider mb-2 block`}>Hall of Fame</span>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Achievements</h2>
          <div className={`h-1 w-16 mx-auto bg-${themeColor}-500 rounded-full`} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {ACHIEVEMENTS_DATA.map((item, idx) => (
            ...
            >
              <div className={`absolute inset-0 bg-${themeColor}-500/10 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              <div className="relative bg-gray-900 border border-gray-800 p-8 rounded-2xl text-center hover:border-gray-600 transition-all h-full flex flex-col items-center">
                <div className="p-4 bg-black rounded-full mb-6 shadow-lg border border-gray-800 group-hover:scale-110 group-hover:border-${themeColor}-500/50 transition-all duration-300">
                  {item.icon}
                </div>
                <h3 className="text-xl font-bold text-white mb-2">{item.title}</h3>
                <p className={`text-${themeColor}-400 text-sm font-medium mb-4`}>{item.org}</p>
...
const PostTimelineSection = ({ themeColor }) => {
  return (
    <div className="bg-black">
      ...
      <section className="py-20 px-6 max-w-6xl mx-auto border-b border-gray-800">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-4">
          ...
          <button className={`px-6 py-2 border border-${themeColor}-500/50 text-${themeColor}-400 rounded-full hover:bg-${themeColor}-500/10 transition-colors text-sm font-medium`}>
            View Gallery
          </button>
        </div>
          ...
          {EVENTS_HIGHLIGHTS.map((event, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                <span className={`text-${themeColor}-400 text-xs font-bold uppercase tracking-wider mb-1`}>{event.role}</span>
      ...
      <section className="py-20 px-6 max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-8">
          {INTERESTS_DATA.map((interest, idx) => (
            ...
            >
              <div className={`w-16 h-16 rounded-2xl bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 group-hover:text-${themeColor}-400 group-hover:border-${themeColor}-500/50 group-hover:-translate-y-2 transition-all duration-300 shadow-lg`}>
                {interest.icon}
              </div>
...
// --- COMPONENT: Modal ---
const Modal = ({ isOpen, onClose, data, themeColor }) => {
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className={`relative w-full max-w-2xl bg-gray-900 border border-${themeColor}-500/30 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]`}
            onClick={(e) => e.stopPropagation()}
          >
              <div className={`h-1 w-full bg-gradient-to-r from-${themeColor}-600 to-${themeColor}-400`} />
            <div className="p-6 md:p-8 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-gray-800/50 p-5 rounded-xl border border-gray-700/50 hover:border-gray-600 transition-colors">
                  <h3 className={`text-${themeColor}-400 font-semibold mb-2 flex items-center gap-2`}><span className="text-xl">🔥</span> The Challenge</h3>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <span key={idx} className={`px-3 py-1 rounded-full text-xs font-medium bg-${themeColor}-900/30 text-${themeColor}-300 border border-${themeColor}-500/30`}>{tech}</span>
                  ))}
                </div>
              </div>
            </div>
...
const TimelineNode = ({ data, index, themeColor, onClick }) => {
  ...
  return (
    ...
    >
      {/* The Dot / Node */}
      <div className={`absolute left-4 md:left-1/2 top-0 w-10 h-10 md:w-12 md:h-12 rounded-full bg-black border-2 flex items-center justify-center z-10 md:-translate-x-1/2 transition-all duration-500 ${isInView ? `border-${themeColor}-500 shadow-[0_0_20px_rgba(var(--${themeColor}-500),0.6)] scale-110` : 'border-gray-700 scale-100'}`}>
        <div className={`transition-colors duration-300 ${isInView ? `text-${themeColor}-400` : 'text-gray-600'}`}>
          {data.icon}
        </div>
        {/* Sparkles/Glow behind */}
        {isInView && (
          <div className={`absolute inset-0 -z-10 bg-${themeColor}-500/30 blur-xl rounded-full animate-pulse`} />
        )}
      </div>
      ...
      <div className={`flex w-full ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} pl-16 md:pl-0`}>
        <div className={`w-full md:w-[45%] ${index % 2 === 0 ? 'md:pr-16 md:text-right' : 'md:pl-16 md:text-left'}`}>
          <div 
            className={`p-6 rounded-2xl border transition-all duration-500 ${isInView ? `bg-gray-900/80 border-${themeColor}-500/30 shadow-[0_0_30px_rgba(0,0,0,0.3)] transform -translate-y-1` : 'bg-gray-900/30 border-gray-800'} hover:bg-gray-800/80`}>
            <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-3 bg-${themeColor}-900/20 text-${themeColor}-400 border border-${themeColor}-500/20`}>{data.year}</span>
            ...
            <div className={`mt-4 flex items-center gap-2 text-xs font-medium text-${themeColor}-400 uppercase tracking-wider ${index % 2 === 0 ? 'md:justify-end' : 'md:justify-start'} opacity-0 group-hover:opacity-100 transition-opacity`}>
...
export default function Portfolio() {
  ...
  // Theme Colors
  const getThemeColor = () => {
    if (persona === 'builder') return 'cyan';
    if (persona === 'leader') return 'orange';
    return 'purple'; // Creator
  };
  ...
  const accentColorClass = t(persona).accent;
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white/20 overflow-x-hidden">
      {/* Tailwind safelist for dynamic theme classes (hidden) */}
      <div className="hidden" aria-hidden>
        <span className="text-cyan-400 text-orange-400 text-purple-400 bg-cyan-900/10 bg-orange-900/10 bg-purple-900/10 border-cyan-500/20 border-orange-500/20 border-purple-500/20 bg-cyan-500/10 bg-orange-500/10 bg-purple-500/10 from-cyan-400 to-cyan-500 from-orange-400 to-orange-500 from-purple-400 to-purple-500 from-cyan-600 to-cyan-400 from-orange-600 to-orange-400 from-purple-600 to-purple-400 shadow-cyan-500/20 shadow-orange-500/20 shadow-purple-500/20 bg-cyan-500/30 bg-orange-500/30 bg-purple-500/30 border-cyan-500/30 border-orange-500/30 border-purple-500/30 shadow-cyan-500/25 shadow-orange-500/25 shadow-purple-500/25" />
      </div>
      
      {/* Progress Bar */}
      <motion.div
        className={`fixed top-0 left-0 right-0 h-1 bg-${themeColor}-500 z-50 origin-left shadow-[0_0_10px_rgba(var(--${themeColor}-500),0.8)]`}
        style={{ scaleX: scaleY }}
      />
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full grid md:grid-cols-2 items-center gap-10">
          ...
          <div className="text-center md:text-left order-2 md:order-1">
            ...
            >
              ...
              I build <span className={`text-transparent bg-clip-text bg-gradient-to-r from-${themeColor}-400 to-white transition-all duration-500`}>journeys.</span>
            </motion.h1>
            ...
            >
              ...
              <a href="https://github.com" target="_blank" rel="noreferrer" className="px-4 py-2 rounded-lg bg-gray-900/80 border border-gray-800 text-gray-300 hover:text-white hover:border-gray-600 transition-all flex items-center gap-2 text-sm font-medium group">
                <Github size={16} className="group-hover:text-white" /> GitHub
              </a>
              <a href="/resume.pdf" className={`px-4 py-2 rounded-lg bg-gray-900/80 border border-${themeColor}-500/30 text-${themeColor}-400 hover:bg-${themeColor}-500/10 transition-all flex items-center gap-2 text-sm font-medium`}>
                <Download size={16} /> CV
              </a>
            </motion.div>

            {/* Persona Selector */}
            <div className="inline-flex bg-gray-900/80 backdrop-blur-sm border border-gray-800 p-1 rounded-xl relative mb-8">
          ...
          <div className="flex justify-center md:justify-end order-1 md:order-2">
            <motion.div
              ...
              className={`relative w-48 h-48 md:w-64 md:h-64 lg:w-80 lg:h-80 rounded-full border-4 border-${themeColor}-500/30 overflow-hidden shadow-2xl shadow-${themeColor}-500/20`}
            >
              <img 
                src="https://api.dicebear.com/9.x/avataaars/svg?seed=Siddharth&backgroundColor=b6e3f4,c0aede,d1d4f9" 
                alt="Siddharth Sharma" 
                className="w-full h-full object-cover hover:scale-110 transition-transform duration-500"
              />
               {/* Glow Effect */}
              <div className={`absolute inset-0 bg-gradient-to-tr from-${themeColor}-500/20 to-transparent pointer-events-none`} />
            </motion.div>
          </div>
        </div>
        
        {/* Persona Summary Card - Full Width Below Grid */}
        <PersonaSummary themeColor={themeColor} persona={persona} />
      ...
      <CharacterStats themeColor={themeColor} persona={persona} />

      {/* Skills Section - Above Timeline */}
      <SkillsSection themeColor={themeColor} persona={persona} onSkillClick={(category) => setSelectedSkillCategory(category)} />
      ...
      <section className="relative py-20 px-6 max-w-5xl mx-auto">
        ...
        <div className="relative">
          ...
          <div ref={timelineRef} className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-gray-800 md:-translate-x-1/2 overflow-hidden">
            <motion.div 
              className={`absolute top-0 left-0 right-0 h-[300px] bg-gradient-to-b from-transparent via-${themeColor}-500 to-transparent opacity-70`}
              animate={{ y: glowRange }}
          ...
          {filteredData.map((item, index) => (
            <TimelineNode key={item.id} data={item} index={index} themeColor={themeColor} onClick={setSelectedNode} />
          ))}
        </div>
      </section>

      {/* Achievements & Extras Section */}
      <AchievementsSection themeColor={themeColor} />
      
      <PostTimelineSection themeColor={themeColor} />

      {/* Contact Section */}
      <section className="py-24 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          <div className={`w-16 h-16 mx-auto bg-${themeColor}-500/10 rounded-full flex items-center justify-center text-${themeColor}-400 mb-6`}>
             <Mail size={32} />
          </div>
          ...
          <a href="mailto:sidd13704@gmail.com" className={`inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-${themeColor}-600 to-${themeColor}-500 hover:from-${themeColor}-500 hover:to-${themeColor}-400 rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-lg shadow-${themeColor}-500/25`}>
      ...
      {/* Modals */}
      <Modal isOpen={!!selectedNode} onClose={() => setSelectedNode(null)} data={selectedNode} themeColor={themeColor} />
